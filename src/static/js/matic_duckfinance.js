...
const DUCK_CHEF_ABI = [
  ...
  },
  {
    ...
    inputs: [
      {indexed: true, internalType: 'address', name: 'user', type: 'address'},
      {indexed: false, internalType: 'uint256', name: 'DuckPerBlock', type: 'uint256'},
    ],
...
async function main() {
  ...
  const DUCK_CHEF_ADDR = '0x91f0e7c56deafe8c2be3398ea7589d62a092daa0'
  const rewardTokenTicker = 'DUCK'
  const DUCK_CHEF = new ethers.Contract(DUCK_CHEF_ADDR, DUCK_CHEF_ABI, App.provider)

  const startBlock = await DUCK_CHEF.startBlock()
  const currentBlock = await App.provider.getBlockNumber()
  const bonusMultiplier = await DUCK_CHEF.BONUS_MULTIPLIER()

  let rewardsPerWeek = 0
  if (currentBlock < startBlock) {
    _print(`Rewards start at block ${startBlock}\n`)
  } else {
    rewardsPerWeek = ((((await DUCK_CHEF.DuckPerBlock()) / 1e18) * 604800) / 2.1) * bonusMultiplier
  }
